    ...
    if (document.getElementById("C-edition").value != 0 && document.getElementById("C-Unit").value != 0) {
        if (document.getElementById("C-pageno").value != 0) {
            if (document.getElementById("C-pager").value != 0) {
                ...
                {
                    unit_name = "Pathanamthitta"
                } if(unit==="TCR")
                {
                    unit_name = "Trissur"
                } if(unit==="TVM")
                {
                    unit_name = "Trivandram"
                } if(unit==="GLF")
                {
                    unit_name = "Gulf"
                } if(unit==="OK")
                ...
                let filename = unit+"-Obit-"+unit_name+"-Manorama-"+edition_name+"-A-"+day+month+year+"-"+pageno
                pdf_filename =filename;
                if (pager === "1") {
                    oddTemplateBuilder(pageno, unit, edition, year, mlMonth, day, mlDay, email,filename)
                }
                if (pager === "2") {
                    evenTemplateBuilder(pageno, unit, edition, year, mlMonth, day, mlDay, email,filename)
                }
                console.log("email", email);
                console.log("template", template_loc);
            } else {
                document.getElementById("error-box").show();
                document.getElementById("ErrorTXT").innerHTML = "Please Select the Page!";
            }
        } else {
            document.getElementById("error-box").show();
            document.getElementById("ErrorTXT").innerHTML = "Please Insert Page Number!";
        }

    } else {
        document.getElementById("error-box").show();
        document.getElementById("ErrorTXT").innerHTML = "Unit or Edition is not Selected! Please Check...";

    }
});

async function oddTemplateBuilder(pageno, unit, edition, year, mlMonth, day, mlDay, email,filename) {
    const {localFileSystem} = require('uxp').storage;
    const {app} = require('indesign');
    const new_doc = await localFileSystem.getEntryWithUrl(template_loc + "\\" + odd_temp_filename);
    if (new_doc) {
        var _path = page_loc+"\\"+filename+".indd"
            app.open(new_doc);
            const doc = app.activeDocument.save(_path)
            oddtemplate(doc,pageno, unit, edition, year, mlMonth, day, mlDay, email);     
            }
        }
    
    async function oddtemplate(doc,pageno, unit, edition, year, mlMonth, day, mlDay, email)
    {
        const EmailFrame = doc.textFrames.add();
                ...
                EmailStyler(Read_Email_style,EmailFrame.parentStory);
                ...
                FunStyler(Read_Unit_style,UnitFrame.parentStory);
...
async function evenTemplateBuilder(pageno, unit, edition, year, mlMonth, day, mlDay, email,filename) {
    ...
    const new_doc = await localFileSystem.getEntryWithUrl(template_loc + "\\" + even_temp_filename);
        if (new_doc) {
            var _path = page_loc+"\\"+filename+".indd"
                app.open(new_doc);
                const doc = app.activeDocument.save(_path)
                eventemplate(doc,pageno, unit, edition, year, mlMonth, day, mlDay, email);     
                }
            }

          async function eventemplate(doc,pageno, unit, edition, year, mlMonth, day, mlDay, email)
          {
        const EmailFrame = doc.textFrames.add();
        const UnitFrame = doc.textFrames.add();
        const EditionFrame = doc.textFrames.add();
...
{
    document.getElementById('printing-bar').value = 0; 
    document.getElementById("print-no").style.display="block";
    document.getElementById("print-yes").style.display="block";
    document.getElementById("print-ok").style.display="none";
    document.getElementById("print-confirm-box").show();
    document.getElementById("confirm-print").innerHTML = "Do you want to Print PDF?"
});


document.getElementById("print-yes").addEventListener("click", function(){
    if(pdf_filename!="undefined")
    {
        let i =0
        const counter = setInterval(()=>{
            i++
            document.getElementById('printing-bar').value = i;
            if(i>=100)
            {
            ...
            document.getElementById("confirm-print").innerHTML = "Completed Please Check "+page_loc;
        ...
        document.getElementById("print-no").style.display="none";
        document.getElementById("print-yes").style.display="none";
        document.getElementById("print-ok").style.display="block";
            }
        else
        {
...
document.getElementById("print-no").addEventListener("click",function(){
    document.getElementById("print-confirm-box").close();
})
document.getElementById("print-ok").addEventListener("click",function(){
    document.getElementById("print-confirm-box").close();
})